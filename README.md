...
git clone https://github.com/YOUR_USERNAME/behaviour-pulse.git
cd behaviour-pulse
...
# SMTP_HOST, SMTP_USER, SMTP_PASS (optional — app works without SMTP using sandbox preview)
...
│   └── ai.js        # Gemini AI endpoints (refine, reflect, parse-roster)
├── models/          # Mongoose schemas & local DB wrappers
├── utils/
│   ├── emailService.js    # SMTP dispatch, Ethereal fallback, AI email generation
│   ├── emailLogStore.js   # Async email audit log store
...
- **Production:** Connect MongoDB via the `MONGODB_URI` environment variable for full concurrent multi-school support.

### React Performance Optimizations
- `filteredStudents`, `sortedStudents`, and audit log computations are all wrapped in `useMemo` and `useCallback` hooks to eliminate keystroke lag on large rosters.
...
|----------|-------------|----------|
| `GEMINI_API_KEY` | Google Gemini API key for AI features | Recommended |
...
> Without SMTP credentials, emails are dispatched to an **Ethereal sandbox** preview — you can view them in the Email Dispatch Log panel inside the app.
