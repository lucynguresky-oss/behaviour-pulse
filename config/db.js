...
async function seedMongooseDb() {
  try {
    ...
    if (schoolCount === 0) {
      ...
      // 2. Hash principal and starter passwords
      const salt = await bcrypt.genSalt(10);
      const hashedPassword = await bcrypt.hash("BarakaNgureNjihia", salt);
      
      // 3. Create starter users
      await MongooseUser.insertMany([
        {
          _id: new mongoose.Types.ObjectId("648000000000000000000002"),
        ...
        {
          _id: new mongoose.Types.ObjectId("648000000000000000000003"),
          schoolId: school._id.toString(),
          name: "Deputy Harris",
        ...
        {
          _id: new mongoose.Types.ObjectId("648000000000000000000004"),
          schoolId: school._id.toString(),
        ...
        {
          _id: new mongoose.Types.ObjectId("648000000000000000000005"),
          schoolId: "global",
        ...
        {
          _id: new mongoose.Types.ObjectId("648000000000000000000006"),
          schoolId: school._id.toString(),
          name: "Alice Smith",
        ...
        {
          _id: new mongoose.Types.ObjectId("648000000000000000000007"),
          schoolId: school._id.toString(),
          name: "Bob Jones",
      ...
      // 4. Create starter logs
      await MongooseBehaviorLog.insertMany([
        {
          schoolId: school._id.toString(),
          studentId: new mongoose.Types.ObjectId("648000000000000000000006"),
          teacherId: new mongoose.Types.ObjectId("648000000000000000000002"),
          pointsChange: 15,
...
export async function connectDB() {
  ...
  if (!uri) {
    ...
    console.log("👉 Using local JSON-file-based database system for local development / instant preview.");
    useLocalDb = true;
  ...
  try {
    ...
    console.log("👉 Falling back to local JSON-file-based database system.");
    useLocalDb = true;
    return { isConnected: false, useLocalDb: true };
  }
}
